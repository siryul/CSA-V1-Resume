...
def main_worker(gpu, ngpus_per_node, config, logger, model_dir):
  ...
  for epoch in range(config.num_epochs):
    ...
    if config.loss_type == 'CE':
      ...
      criterion = LogitAdjust(cls_num_list=cls_num_list)
    elif config.loss_type == 'DFL':
      criterion = DFL()
    elif config.loss_type == 'BCL':
      criterion = {
        "lc": LogitAdjust(cls_num_list=cls_num_list),
        "bcl": BalSCL(cls_num_list=cls_num_list)
      }
    else:
...
def train(train_loader,
  ...
  fit_num = 0
  tot_num = 0
  back_images = torch.Tensor([])
  back_masks = torch.Tensor([])
  ...
  if isinstance(model, torch.nn.DataParallel):
    ...
    for i in model.device_ids:
      if i in feat_map_global.keys():
        feat_map.append(feat_map_global[i])
        grad_map.append(grad_map_global[i])
...
class AccMeter:
  ...
  def get_shot_acc(self):
    ...
    # print(f'acc_classes: {acc_classes}')
